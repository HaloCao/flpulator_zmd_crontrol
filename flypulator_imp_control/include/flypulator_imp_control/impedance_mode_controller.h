#ifndef IMPEDANCE_MODE_CONTROLLER_H
#define IMPEDANCE_MODE_CONTROLLER_H

#include "flypulator_imp_control/base_controller.h"
#include <eigen3/Eigen/Dense>

class ImpedanceModeController : public BaseController
{
public:
  ImpedanceModeController()
  {
    ROS_ERROR("Impedance mode Controller, dont know why this constructor has been called");
  };
  // initialize class variables in constructor with initialization list as best practise
  ImpedanceModeController(const float mass, const Eigen::Matrix3f inertia, const float gravity)
    : mass_(mass)
    , inertia_(inertia)
  ...
  void configCallback(flypulator_imp_control::imp_parameterConfig& config, uint32_t level);
...
#endif  // Impedance_MODE_CONTROLLER_H
