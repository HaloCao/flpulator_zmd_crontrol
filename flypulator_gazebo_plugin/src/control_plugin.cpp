...
public:
  virtual void Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
  {
    ...
    if (!ros::isInitialized())
    {
      int argc = 0;
      char **argv = NULL;
      ros::init(argc, argv, "gazebo_control", ros::init_options::NoSigintHandler);
    ...
    common::PID controller_translation_xy(linear_p, linear_i, linear_d, linear_imax, -linear_imax, _maxForce,
                                          -_maxForce);
    common::PID controller_rotation_xy(angular_p, angular_i, angular_d, angular_imax, -angular_imax, _maxTorque,
                                       -_maxTorque);
    ...
    common::PID controller_translation_z(linear_p, linear_i, linear_d, linear_imax, -linear_imax, _maxForce,
                                         -_maxForce);
    common::PID controller_rotation_z(angular_p * 2.0, angular_i * 2.0, angular_d * 2.0, angular_imax, -angular_imax,
                                      _maxTorque, -_maxTorque);
    ...
    ros::SubscribeOptions s1 = ros::SubscribeOptions::create<geometry_msgs::Vector3>(
        "/drone/wind_cmd", 100, boost::bind(&ControlPlugin::OnRosWindMsg, this, _1), ros::VoidPtr(), &this->rosQueue);
    ...
    ros::SubscribeOptions s2 = ros::SubscribeOptions::create<gazebo_msgs::LinkStates>(
        "/gazebo/link_states", 100, boost::bind(&ControlPlugin::OnlinkMsg, this, _1), ros::VoidPtr(), &this->rosQueue);
    this->rosSubLink = this->rosNode->subscribe(s2);

    // subscribe to control signal,six global velocity for drone
    ros::SubscribeOptions s3 = ros::SubscribeOptions::create<geometry_msgs::Wrench>(
        "/drone/vel_cmd", 100, boost::bind(&ControlPlugin::OnControlMsg, this, _1), ros::VoidPtr(), &this->rosQueue);
    this->rosSubControl = this->rosNode->subscribe(s3);

    ros::SubscribeOptions s4 = ros::SubscribeOptions::create<flypulator_common_msgs::Vector6dMsg>(
        "/drone/thrust_moment_ratio", 100, boost::bind(&ControlPlugin::OnRosRatioMsg, this, _1), ros::VoidPtr(),
        &this->rosQueue);
    ...
    this->updateConnection = event::Events::ConnectWorldUpdateBegin(boost::bind(&ControlPlugin::OnUpdate, this, _1));
...
public:
  ...
  {
    // double modelLinearVel_x, modelLinearVel_y, modelLinearVel_z, modelAngularVel_x, modelAngularVel_y,
    // modelAngularVel_z; double linearError_x, linearError_y, linearError_z, angularError_x, angularError_y,
    // angularError_z;
...
public:
  ...
  {
    static int control_loop_cnt = 0;

    if (control_loop_cnt >= 1)
    {
      control_loop_cnt = 0;
      double modelLinearVel_x, modelLinearVel_y, modelLinearVel_z, modelAngularVel_x, modelAngularVel_y,
          modelAngularVel_z;
      ...
      ***********************************************************************/
      ...
      ros::spinOnce();
      // ROS_INFO_STREAM("control:"<<_rotor_cmd.velocity[0]<<","<<_rotor_cmd.velocity[1]<<","<<_rotor_cmd.velocity[2]<<","<<_rotor_cmd.velocity[3]<<","<<_rotor_cmd.velocity[4]<<","<<_rotor_cmd.velocity[5]);
    }
    else
      control_loop_cnt++;
  }
