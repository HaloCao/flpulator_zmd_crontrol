...
extern const QMetaObject staticMetaObject;  // in moc-run we create a static meta object for QCP "fake" object. This
                                            // line is the link to it via QCP::staticMetaObject in normal operation as
                                            // namespace
...
void QCPDataContainer<DataType>::add(const QCPDataContainer<DataType> &data)
{
  ...
  if (oldSize > 0 && !qcpLessThanSortKey<DataType>(*constBegin(), *(data.constEnd() - 1)))  // prepend if new data keys
                                                                                            // are all smaller than or
                                                                                            // equal to existing ones
  {
    if (mPreallocSize < n)
      preallocateGrow(n);
    mPreallocSize -= n;
...
void QCPDataContainer<DataType>::add(const QVector<DataType> &data, bool alreadySorted)
{
  ...
  if (alreadySorted && oldSize > 0 &&
      !qcpLessThanSortKey<DataType>(*constBegin(), *(data.constEnd() - 1)))  // prepend if new data is sorted and keys
...
void QCPDataContainer<DataType>::performAutoSqueeze()
{
  ...
  else if (totalAlloc > 1000)  // below 10 MiB raw data be generous with preallocated memory, below 1k points don't even
                               // bother
  {
...
double QCPAbstractPlottable1D<DataType>::selectTest(const QPointF &pos, bool onlySelectable, QVariant *details) const
{
  ...
  for (typename QCPDataContainer<DataType>::const_iterator it = begin; it != end; ++it)
  {
    ...
    if (keyRange.contains(mainKey) && valueRange.contains(mainValue))  // make sure data point is inside visible range,
...
void QCPAbstractPlottable1D<DataType>::drawPolyline(QCPPainter *painter, const QVector<QPointF> &lineData) const
{
      ...
      !painter->modes().testFlag(QCPPainter::pmVectorized) && !painter->modes().testFlag(QCPPainter::pmNoCaching))
  {
    ...
    while (i < lineDataSize && (qIsNaN(lineData.at(i).y()) || qIsNaN(lineData.at(i).x())))  // make sure first point is
                                                                                            // not NaN
      ++i;
...
protected:
  ...
  const QPolygonF getChannelFillPolygon(const QVector<QPointF> *lineData, QCPDataRange thisSegment,
